...
vi.mock('@/services/IndexedDBService', () => {
  class FakeDB {
    ...
    async getData(key: string) {
      const f = this.stores.settings.find(i => i.key === key)
      return f?.value ?? null
    }
    async getAllData(store: string) {
      return this.exportDB(store)
    }
    async getDataFromStore(store: string, key: string) {
      return (this.stores[store] || []).find(i => i.id === key || i.activityId === key)
    }
...
describe('StorageService fusion logique', () => {
  ...
  it('importe items distants manquants localement', async () => {
    ...
    const _written: Record<string, any[]> = {}
    mgr.setPlugins([
      {
        id: 'mock',
        label: 'Mock',
    ...
    mgr.setPlugins([
      {
        ...
        writeRemote: async (_store: string, merged: any[]) => {
          pushed['activities'] = merged
        }
      }
    ])
    await svc.syncStores([{ store: 'activities', key: '' }])
