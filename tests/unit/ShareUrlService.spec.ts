...
describe('ShareUrlService', () => {
  ...
  describe('isValidManifestUrl', () => {
    ...
    it('should accept localhost URLs in development mode', () => {
      ...
      expect(ShareUrlService.isValidManifestUrl('http://localhost:3000/test-manifest.json')).toBe(
        true
      )
      expect(ShareUrlService.isValidManifestUrl('http://127.0.0.1:3000/test-manifest.json')).toBe(
        true
      )
    })
  })
