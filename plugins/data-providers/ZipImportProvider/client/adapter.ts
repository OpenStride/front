...
interface ZipRawData extends RawRecord {
  records?: RawRecord[]
  samples?: RawRecord[]
  laps?: RawRecord[]
  activity?: RawRecord
...
export function adaptZipSummary(zipDetails: ZipRawData): Activity {
  // Conversion date en timestamp integer
  function toTimestamp(date: unknown): number {
    if (!date) return 0
    if (typeof date === 'number') return date
  ...
  for (let i = 0; i < samples.length; i += step) {
    ...
    if (s.start_position_lat && s.start_position_long) {
      polyline.push([s.start_position_lat as number, s.start_position_long as number])
    } else if (s.position_lat && s.position_long) {
      polyline.push([s.position_lat as number, s.position_long as number])
    }
  }
  const startRaw =
    zipDetails['Activity Date'] ||
    zipDetails.activity?.timestamp ||
    zipDetails.sessions?.[0]?.start_time
  return {
    id: `zip_${zipDetails['Activity ID']}`,
    provider: 'zip',
    startTime: toTimestamp(startRaw),
    duration: Number(
      zipDetails['Elapsed Time_1'] ||
        zipDetails['Elapsed Time'] ||
        zipDetails.activity?.total_timer_time ||
        zipDetails.sessions?.[0]?.total_timer_time
    ),
    distance: Number(
      zipDetails['Distance_1'] ||
        zipDetails['Distance'] ||
        zipDetails.activity?.total_distance ||
        zipDetails.sessions?.[0]?.total_distance
    ),
    type: String(
      zipDetails['Activity Type'] ||
        zipDetails.activity?.type ||
        zipDetails.sessions?.[0]?.sport ||
        'unknown'
    ).toLowerCase(),
    title: String(
      zipDetails['Activity Name'] ||
        zipDetails.activity?.name ||
        zipDetails.sessions?.[0]?.sport ||
        ''
    ),
...
export function adaptZipDetails(zip: ZipRawData): ActivityDetails {
  // Start time: CSV date, FIT session start_time
  const start = toTimestamp(
    zip['Activity Date'] || zip.sessions?.[0]?.start_time || zip.activity?.timestamp || 0
  )
  ...
  return {
    ...
    stats: {
      averageHeartRate: Number(
        zip['Average Heart Rate'] ?? zip.sessions?.[0]?.avg_heart_rate ?? null
      ),
