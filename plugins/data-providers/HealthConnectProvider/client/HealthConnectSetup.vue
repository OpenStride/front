<template>
  <div class="max-w-md mx-auto p-6 bg-white rounded-xl shadow space-y-4">
    <h2 class="text-xl font-semibold text-center">Health Connect</h2>
    ...
    <div class="text-center mt-4">
      ...
      >
        ...
        {{
          isConnected
            ? 'Connecté à Health Connect'
            : 'Importer vos entrainements depuis Health Connect'
        }}
      </p>
    </div>
    ...
    <div class="flex flex-wrap justify-center gap-3 mt-6">
      ...
      >
        ...
        {{ isLoading ? 'Connexion...' : 'Autoriser Health Connect' }}
      </button>
...
async function requestAccess() {
  ...
  try {
    ...
    notifications.notify('Health Connect: plugin natif non encore configuré', { type: 'warning' })
  } catch (err: any) {
    error.value = err.message || "Erreur d'autorisation Health Connect"
  } finally {
...
async function importWorkouts() {
  ...
  try {
    ...
    notifications.notify('Health Connect: import non encore implémenté', { type: 'warning' })
  } catch (err: any) {
