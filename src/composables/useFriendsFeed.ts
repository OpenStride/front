...
/**
 * Composable for managing friends-only activity feed state
 * Filters activities to show only friend activities
...
export function useFriendsFeed() {
  ...
  /**
   * Load all activities from service and filter friends only
   */
  ...
  const loadMore = async () => {
    ...
    loadMorePromise = (async () => {
      ...
      try {
        ...
        console.error('[useFriendsFeed] Error loading activities:', error)
      } finally {
        loading.value = false
