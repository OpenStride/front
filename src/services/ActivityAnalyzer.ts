...
export class ActivityAnalyzer {
    ...
    public sampleByLaps(
        ...
        for (const lap of laps) {
            ...
            while (
                sampleIdx < this.samples.length &&
                this.samples[sampleIdx].time < lap.time
            ) {
  ...
  */
    public sampleBySlopeChange(minDistanceMeters: number): Sample[] {
        ...
        const elevSmooth = this.samples.map((_, i) => {
            let sum = 0, cnt = 0;
            for (let j = Math.max(0, i - SMOOTH_WINDOW);
        ...
        for (let i = 1; i < this.samples.length; i++) {
            ...
            if (segDir === null) { segDir = dir; continue; }

            // même sens ⇒ reset candidat
            if (dir === segDir || dir === 0) { candIdx = -1; candDir = null; continue; }

            // nouvelle direction
            if (candIdx === -1) { candIdx = i; candDir = dir; continue; }

            // bruit ? → redéfinit candidat
            if (dir !== candDir) { candIdx = i; candDir = dir; continue; }
    ...
    > {
        ...
        const valid = this.samples.every(
            s => s.distance != null && s.time != null
        )
        if (!valid) {
            throw new Error(
                'Tous les samples doivent contenir une distance et un temps cumulés.'
            )
        }
