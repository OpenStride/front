...
export class InteractionService {
  ...
  private triggerAutoPublish(): void {
    ...
    this.publishDebounceTimer = setTimeout(async () => {
      try {
        ...
        if (result.success) {
          console.log(
            `[InteractionService] Auto-published ${result.interactionsSynced} interactions`
          )
        }
      } catch (error) {
  ...
  public async addLike(activityId: string, activityOwnerId: string): Promise<Interaction | null> {
    ...
    console.log(
      `[InteractionService] Like added on activity ${activityId} (mutual friend confirmed)`
    )
    return interaction
  }
  ...
  public async addComment(
    ...
    console.log(
      `[InteractionService] Comment added on activity ${activityId} (mutual friend confirmed)`
    )
    return interaction
  }
  ...
  public async deleteComment(interactionId: string): Promise<void> {
    ...
    // Get the interaction
    const interaction = (await db.getDataFromStore(
      'interactions',
      interactionId
    )) as Interaction | null
