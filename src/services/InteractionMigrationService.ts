...
export async function migrateInteractionOwnerIds(): Promise<{
  ...
  console.log(
    `[InteractionMigration] Found ${friends.length} friends and ${interactions.length} interactions`
  )
  ...
  for (const friend of friends) {
    if (friend.userId) {
      ...
      console.log(
        `[InteractionMigration] Mapping: ${friend.id} -> ${friend.userId} (${friend.username})`
      )
    }
  }
  ...
  for (const interaction of interactions) {
    ...
    if (newOwnerId && newOwnerId !== interaction.activityOwnerId) {
      ...
      updated++
      console.log(
        `[InteractionMigration] Updating interaction ${interaction.id}: ${interaction.activityOwnerId} -> ${newOwnerId}`
      )
    } else if (!newOwnerId && interaction.activityOwnerId.startsWith('friend_')) {
      // Old format but no mapping available
      console.warn(
        `[InteractionMigration] No mapping for ${interaction.activityOwnerId} (interaction ${interaction.id})`
      )
      skipped++
    } else {
  ...
  console.log(
    `[InteractionMigration] Migration complete: ${updated} updated, ${skipped} skipped, ${errors.length} errors`
  )
...
export async function debugInteractionIds(): Promise<void> {
  ...
  for (const interaction of interactions) {
    groupedByOwner[interaction.activityOwnerId] =
      (groupedByOwner[interaction.activityOwnerId] || 0) + 1
  }
