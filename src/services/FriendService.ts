...
export class FriendService {
    ...
    try {
      ...
      if (interactionResult.success && interactionResult.interactionsSynced > 0) {
        ...
        if (interactionYears.length > 0) {
          ...
          console.log(`[FriendService] Added ${interactionYears.length} interaction years to manifest`);
        }
      }
  ...
  private async checkMutualFriendship(friend: Friend, manifest: PublicManifest): Promise<boolean> {
    ...
    if (followsMe !== friend.followsMe) {
      ...
      if (followsMe) {
        console.log(`[FriendService] Mutual friendship discovered: ${friend.username} follows me back!`);
        this.emitEvent({
        ...
        console.log(`[FriendService] Mutual friendship ended: ${friend.username} no longer follows me`);
      }
    }
  ...
  public async syncFriendActivitiesQuick(
    ...
    try {
      ...
      if (manifest.profile.userId && !friend.userId) {
        ...
        await db.addItemsToStore('friends', [friend], (f) => f.id);
        console.log(`[FriendService] Updated friend ${friendId} with stable userId: ${friend.userId}`);
      }
