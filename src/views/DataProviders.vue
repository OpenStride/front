<template>
  <div class="max-w-3xl mx-auto py-10 px-4 space-y-10">
    <section data-test="connected-providers-section">
      <h1 class="text-2xl font-bold mb-4" data-test="connected-providers-title">
      ...
      <p v-else class="text-gray-500" data-test="no-providers-message">
        {{ t('dataProviders.noProviders') }}
      </p>
    </section>

    <section data-test="available-providers-section">
      <h2 class="text-xl font-semibold mb-4" data-test="available-providers-title">
...
const availableProviders = computed(() =>
  installableProviderPlugins.filter(p => !userProviders.value.some(up => up.id === p.id))
)
