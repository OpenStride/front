<template>
  <div class="friends-page">
    <div class="page-header">
      <h2 class="page-title">Mes Amis</h2>
...
const loadFriends = async () => {
  ...
  try {
    ...
    console.error('[FriendsPage] Error loading friends:', error);
  } finally {
    loading.value = false;
  }
};
...
const refreshAll = async () => {
  ...
  try {
    ...
    console.error('[FriendsPage] Error refreshing friends:', error);
  } finally {
...
const refreshFriend = async (friendId: string) => {
  ...
  try {
    ...
    console.error('[FriendsPage] Error refreshing friend:', error);
  } finally {
...
const syncAllActivities = async (friendId: string) => {
  ...
  try {
    ...
    if (result.success) {
      console.log(`[FriendsPage] Full sync completed: ${result.activitiesAdded} new, ${result.totalActivities} total`);
    } else {
      console.error('[FriendsPage] Full sync failed:', result.error);
    }

    await loadFriends();
  } catch (error) {
    console.error('[FriendsPage] Error syncing all activities:', error);
  } finally {
